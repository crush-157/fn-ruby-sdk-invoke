...
def compartments
  id_client.list_compartments(OCI.config.tenancy, limit: 50, compartment_id_in_subtree: true)
    .collect {|r| r.data }
    .flatten
end
...
def compartment_ocid(compartment_name:)
  compartments.select { |c| c.name == compartment_name }
...
def apps(compartment_name:)
  fn_management_client.list_applications(compartment_ocid(compartment_name: compartment_name), limit: 50)
    .collect {|r| r.data }
    .flatten
end
...
# Returns the list of functions in the named application in the named compartment
def functions(app_name:, compartment_name:)
  fn_management_client.list_functions(app_ocid(app_name: app_name, compartment_name: compartment_name), limit: 50)
    .collect { |r| r.data }
    .flatten
end

# Returns the OCID of the named function in the named application in the named compartment
